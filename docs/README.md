...
Este arquivo fornece contexto e instruções deste repositório de automação do jogo **Screeps**.
...
## 📁 Estrutura de Documentação Lógica

Para entender detalhadamente o funcionamento de cada módulo, consulte os guias abaixo:

- **[Lógica do Spawn](spawns/logic.md)**: Gerenciamento de população, prioridades e escalonamento de corpo.
- **[Lógica das Roles](roles/logic.md)**: Comportamentos individuais, estados e persistência de alvos.
- **[Lógica das Torres](structures/tower_logic.md)**: Defesa ativa, manutenção de estruturas e economia de energia.

## 📁 Estrutura de Arquivos do Projeto (Código)

- `src/main.ts`: Loop principal e orquestração global.
- `src/structures/structure*.ts`: Inteligência de planejamento de estruturas, com verificação e reconstrução, ignorando áreas inseguras.
- `src/rooms/room*.ts`: Inteligência de planejamento de rooms.
- `src/spawns/spawn*.ts`: Inteligência de geração de creeps.
- `src/tools/tool*.ts`: Ferramentas em geral.
-  `src/roles/role*.ts`: Comportamentos específicos de creeps com persistência de alvo na memória para evitar oscilações.
- `src/tasks/task*.ts`: Módulos de tarefas reutilizáveis (Build, Upgrade, CollectEnergy, Repair).
