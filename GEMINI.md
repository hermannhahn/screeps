...
- [ ] Implementar `RemoteRoomManager` para automação de expansão, atribuição de salas, coordenação de creeps e planejamento de infraestrutura em salas remotas.
- [ ] Refatorar a função de priorização de creeps hostis (`findPrioritizedHostileCreep`) para um módulo de utilidade (`src/utils.combat.ts`) para evitar duplicação em `role.guard.ts`, `role.archer.ts` e `manager.tower.ts`.
- [ ] Melhorar `role.reserver.ts` para verificar proativamente o tempo de reserva do controller e renová-lo.
- [ ] Melhorar `role.remoteHarvester.ts` para lidar com creeps hostis na sala alvo (e.g., fugir ou notificar a sala de origem).
- [ ] Estender `rampartsWallsBlueprint` para planejar explicitamente `STRUCTURE_WALL`s, além de `STRUCTURE_RAMPART`s.
- [ ] Melhorar a lógica de planejamento de `rampartsWallsBlueprint` para incluir considerações estratégicas (pontos de estrangulamento, defesa em camadas).
- [ ] Implementar lógica de manutenção de pontos de vida elevados para Walls e Ramparts (milhões de hits) em `manager.tower.ts` e `task.repair.ts`, com thresholds escaláveis por RCL.
- [ ] Criar pontos de rally defensivos mais sofisticados para `role.guard.ts` e `role.archer.ts`.
- [ ] Considerar a implementação de creeps dedicados a cura ou lógica de cura mais avançada para torres.
